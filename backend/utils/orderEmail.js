      ...
      <div style="padding:28px 32px;background:white">
        ...
        <div style="background:#f7f5f0;border-radius:10px;padding:16px 18px;margin-bottom:20px">
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            ...
            <span style="font-size:13px;color:${order.shippingPrice === 0 ? '#27ae60' : '#1a1a2e'}">${order.shippingPrice === 0 ? 'FREE' : `₹${order.shippingPrice}`}</span>
          </div>
...
const sendStatusUpdateEmail = async (order, user) => {
  const STATUS_CONFIG = {
    ...
    delivered:  { icon: '✅', color: '#27ae60', title: 'Order Delivered!',          message: 'Your order has been delivered successfully. Enjoy your purchase!' },
